<%@ Control Language="C#" ClassName="TableData" %>
<%
  // TableData.ascx
  // Implementation of the AJAX enabled TableData Control.
  // Copyright (c) by Matthias Hertel, http://www.mathertel.de
  // This work is licensed under a BSD style license. See http://www.mathertel.de/License.aspx
  // ----- 
  // 01.06.2005 created by Matthias Hertel
  // 06.11.2005 cols attribute added.
  // 12.11.2005 FireFox compatibility: white space removed
%>
...
<script runat="server">
  ...
  protected override void OnPreRender(EventArgs e) {
    ...
    if (!Page.ClientScript.IsClientScriptBlockRegistered(this.GetType(), "MyBehaviour")) {
      ...
      ((HtmlHead)Page.Header).Controls.Add(new LiteralControl("<script type='text/javascript' src='"
        + Page.ResolveUrl("~/controls/tabledata.js")
        + "'><" + "/script>\n"));
    } // if
  ...
  <div id="<%=this.UniqueID %>" class="TableData" style="overflow-y: scroll; height: 420px"><table style="table-layout:fixed">
  <%
...
jcl.LoadBehaviour("<%=this.UniqueID %>", TableDataBehaviour);
    </script>
